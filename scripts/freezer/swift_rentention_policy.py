...
def set_expiration_on_container(conn, container, delete_timestamp, verbose=False):
    ...
    try:
        # Paginate through all objects using marker, since Swift returns
        # at most 10,000 objects per request.
        objects = []
        marker = ""
        while True:
            _, page = conn.get_container(container, marker=marker)
            if not page:
                break
            objects.extend(page)
            marker = page[-1]["name"]
        ...
        if object_count == 0:
            print("WARNING: Container is already empty")
            return 0, 0
        ...
        for i, obj in enumerate(objects, 1):
            ...
            try:
                ...
                if verbose:
                    print(f"OK [{i}/{object_count}] {obj_name}")

                succeeded += 1
            except ClientException as e:
                if e.http_status == 404:
                    if verbose:
                        print(f"WARNING [{i}/{object_count}] Skipped (404): {obj_name}")
                    succeeded += 1  # Don't count as failure
                else:
                    print(f"FAIL [{i}/{object_count}] Failed: {obj_name} - {e}")
                    failed += 1
            except Exception as e:
                print(f"FAIL [{i}/{object_count}] Error: {obj_name} - {e}")
                failed += 1
        ...
        print()
        print(f"OK Expiration set on {succeeded} objects")
        if failed > 0:
            print(f"FAIL Failed: {failed} objects")

        return succeeded, failed

    except ClientException as e:
        print(f"FAIL Error accessing container: {e}")
        return 0, 0
...
def monitor_and_cleanup(
    ...
    while containers_to_monitor:
        ...
        for container in containers_checked:
            ...
            if not exists:
                print(f"OK {container}: Already deleted")
                containers_to_monitor.remove(container)
                continue

            if obj_count == 0:
                print(f"OK {container}: Empty - deleting...")
                ...
                print(f"WAITING {container}: {obj_count} objects remaining")
...
Examples:
  ...
  %(prog)s -c freezer-bkp-lvm -m 5 --cleanup --check-interval 30
    ...
    if args.dry_run:
        print("\nWARNING: DRY RUN - Exiting without making changes")
        sys.exit(0)
