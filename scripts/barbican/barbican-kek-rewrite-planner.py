...
--set (see the sibling kek.sh library, which also performs the
equivalent of --adopt in bash when an existing Barbican has no kek
configured). This planner stages a rotation by updating that Secret; the
rotation itself happens on the NEXT install-barbican.sh run, when the
db-sync job rewraps every project KEK in the barbican DB (one-way).
