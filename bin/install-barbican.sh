...
S_BARBICAN=$(get_or_create_secret "$SERVICE_NAMESPACE" "barbican-admin" "password" 32 "$ROTATE_SECRETS")
S_DB_ROOT=$(get_or_create_secret "$SERVICE_NAMESPACE" "mariadb" "root-password" 32 "$ROTATE_SECRETS")
S_BARBICAN_DB=$(get_or_create_secret "$SERVICE_NAMESPACE" "barbican-db-password" "password" 32 "$ROTATE_SECRETS")
S_RABBIT_ADMIN=$(get_or_create_secret "$SERVICE_NAMESPACE" "rabbitmq-default-user" "password" 32 "$ROTATE_SECRETS")
S_BARBICAN_RABBIT=$(get_or_create_secret "$SERVICE_NAMESPACE" "barbican-rabbitmq-password" "password" 64 "$ROTATE_SECRETS")
...
set_args=(
    ...
    --set "endpoints.identity.auth.barbican.password=$S_BARBICAN"
    --set "endpoints.oslo_db.auth.admin.password=$S_DB_ROOT"
    --set "endpoints.oslo_db.auth.barbican.password=$S_BARBICAN_DB"
    --set "endpoints.oslo_messaging.auth.admin.password=$S_RABBIT_ADMIN"
    --set "endpoints.oslo_messaging.auth.barbican.password=$S_BARBICAN_RABBIT"
    --set "endpoints.oslo_cache.auth.memcache_secret_key=$S_MEMCACHE"
    --set "conf.barbican.keystone_authtoken.memcache_secret_key=$S_MEMCACHE"
)
...
if "${helm_command[@]}" "${HELM_PASS_THROUGH[@]}"; then
    echo "Helm upgrade successful. Waiting for Barbican deployments..."
    kubectl -n "$SERVICE_NAMESPACE" wait --for=condition=available --timeout=300s \
        deployment/barbican-api \
        deployment/barbican-worker \
        deployment/barbican-keystone-listener
