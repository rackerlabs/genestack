...
S_HEAT=$(get_or_create_secret "$SERVICE_NAMESPACE" "heat-admin" "password" 32 "$ROTATE_SECRETS")
S_TRUSTEE=$(get_or_create_secret "$SERVICE_NAMESPACE" "heat-trustee" "password" 32 "$ROTATE_SECRETS")
S_STACK_USER=$(get_or_create_secret "$SERVICE_NAMESPACE" "heat-stack-user" "password" 32 "$ROTATE_SECRETS")
S_DB_ROOT=$(get_or_create_secret "$SERVICE_NAMESPACE" "mariadb" "root-password" 32 "$ROTATE_SECRETS")
S_HEAT_DB=$(get_or_create_secret "$SERVICE_NAMESPACE" "heat-db-password" "password" 32 "$ROTATE_SECRETS")
...
set_args=(
    ...
    --set "endpoints.identity.auth.heat.password=$S_HEAT"
    --set "endpoints.identity.auth.heat_trustee.password=$S_TRUSTEE"
    --set "endpoints.identity.auth.heat_stack_user.password=$S_STACK_USER"
    --set "endpoints.oslo_db.auth.admin.password=$S_DB_ROOT"
    --set "endpoints.oslo_db.auth.heat.password=$S_HEAT_DB"
    --set "endpoints.oslo_cache.auth.memcache_secret_key=$S_MEMCACHE"
    --set "conf.heat.keystone_authtoken.memcache_secret_key=$S_MEMCACHE"
    --set "endpoints.oslo_messaging.auth.admin.password=$S_RABBIT_ADMIN"
    --set "endpoints.oslo_messaging.auth.heat.password=$S_HEAT_RABBIT"
)
...
if "${helm_command[@]}" "${HELM_PASS_THROUGH[@]}"; then
    echo "Helm upgrade successful. Waiting for Heat deployments..."
    kubectl -n "$SERVICE_NAMESPACE" wait --for=condition=available --timeout=300s \
        deployment/heat-api \
        deployment/heat-api-cfn \
        deployment/heat-engine \
        deployment/heat-api-waitcondition
