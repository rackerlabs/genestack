...
set_args=(
    ...
    --set "endpoints.identity.auth.skyline.password=$(kubectl --namespace openstack get secret skyline-admin -o jsonpath='{.data.password}' | base64 -d)"
    --set "endpoints.oslo_db.auth.admin.password=$(kubectl --namespace openstack get secret mariadb -o jsonpath='{.data.root-password}' | base64 -d)"
    --set "endpoints.oslo_db.auth.skyline.password=$(kubectl --namespace openstack get secret skyline-db-password -o jsonpath='{.data.password}' | base64 -d)"
)
