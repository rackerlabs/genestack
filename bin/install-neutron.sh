#!/bin/bash
# Description: Fetches the version for Neutron and executes helm upgrade.
...
# OVN Endpoint Discovery
OVN_NB_EP=$(kubectl -n kube-system get svc kube-ovn-pnb -o jsonpath='{.spec.clusterIP}:6641' 2>/dev/null || echo "tcp:127.0.0.1:6641")
OVN_SB_EP=$(kubectl -n kube-system get svc kube-ovn-psb -o jsonpath='{.spec.clusterIP}:6642' 2>/dev/null || echo "tcp:127.0.0.1:6642")
...
S_NEUTRON_DB=$(get_or_create_secret "$SERVICE_NAMESPACE" "neutron-db-password" "password" 32 "$ROTATE_SECRETS")
...
set_args=(
    ...
    --set "endpoints.oslo_db.auth.neutron.password=$S_NEUTRON_DB"
    --set "endpoints.oslo_cache.auth.memcache_secret_key=$S_MEMCACHE"
    --set "conf.neutron.keystone_authtoken.memcache_secret_key=$S_MEMCACHE"
    --set "endpoints.oslo_messaging.auth.admin.password=$S_RABBIT_ADMIN"
    --set "endpoints.oslo_messaging.auth.neutron.password=$S_NEUTRON_RABBIT"
    --set "conf.neutron.ovn.ovn_nb_connection=$OVN_NB_EP"
    --set "conf.neutron.ovn.ovn_sb_connection=$OVN_SB_EP"
    --set "conf.plugins.ml2_conf.ovn.ovn_nb_connection=$OVN_NB_EP"
    --set "conf.plugins.ml2_conf.ovn.ovn_sb_connection=$OVN_SB_EP"
)
