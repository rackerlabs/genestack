...
# Note: Many OpenStack services use a specific memcache_secret_key for coordination.
...
set_args=(
    --set "auth.memcache_secret_key=$S_MEMCACHE"
)
...
if execute_helm_upgrade helm_command HELM_PASS_THROUGH; then
    echo "Helm upgrade successful. Waiting for Memcached StatefulSet..."
    wait_for_resource_ready "$SERVICE_NAMESPACE" statefulset 300 "$SERVICE_NAME_DEFAULT"
