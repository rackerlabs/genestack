...
S_GLANCE=$(get_or_create_secret "$SERVICE_NAMESPACE" "glance-admin" "password" 32 "$ROTATE_SECRETS")
S_DB_ROOT=$(get_or_create_secret "$SERVICE_NAMESPACE" "mariadb" "root-password" 32 "$ROTATE_SECRETS")
S_GLANCE_DB=$(get_or_create_secret "$SERVICE_NAMESPACE" "glance-db-password" "password" 32 "$ROTATE_SECRETS")
...
set_args=(
    ...
    --set "endpoints.identity.auth.glance.password=$S_GLANCE"
    --set "endpoints.oslo_db.auth.admin.password=$S_DB_ROOT"
    --set "endpoints.oslo_db.auth.glance.password=$S_GLANCE_DB"
    --set "endpoints.oslo_cache.auth.memcache_secret_key=$S_MEMCACHE"
    --set "conf.glance.keystone_authtoken.memcache_secret_key=$S_MEMCACHE"
    --set "endpoints.oslo_messaging.auth.admin.password=$S_RABBIT_ADMIN"
    --set "endpoints.oslo_messaging.auth.glance.password=$S_GLANCE_RABBIT"
)
...
if execute_helm_upgrade helm_command HELM_PASS_THROUGH; then
    echo "Helm upgrade successful. Waiting for Glance deployments..."
    wait_for_resource_ready "$SERVICE_NAMESPACE" deployment 300 glance-api
