...
SERVICE_NAME_DEFAULT="cloudkitty"
...
S_CLOUDKITTY=$(get_or_create_secret "$SERVICE_NAMESPACE" "cloudkitty-admin" "password" 32 "$ROTATE_SECRETS")
S_DB_ROOT=$(get_or_create_secret "$SERVICE_NAMESPACE" "mariadb" "root-password" 32 "$ROTATE_SECRETS")
S_CLOUDKITTY_DB=$(get_or_create_secret "$SERVICE_NAMESPACE" "cloudkitty-db-password" "password" 32 "$ROTATE_SECRETS")
...
S_CLOUDKITTY_RABBIT=$(get_or_create_secret "$SERVICE_NAMESPACE" "cloudkitty-rabbitmq-password" "password" 64 "$ROTATE_SECRETS")

set_args=(
    --set "endpoints.identity.auth.admin.password=$S_KEYSTONE"
    --set "endpoints.identity.auth.cloudkitty.password=$S_CLOUDKITTY"
    --set "endpoints.oslo_db.auth.admin.password=$S_DB_ROOT"
    --set "endpoints.oslo_db.auth.cloudkitty.password=$S_CLOUDKITTY_DB"
    --set "endpoints.oslo_cache.auth.memcache_secret_key=$S_MEMCACHE"
    --set "conf.cloudkitty.keystone_authtoken.memcache_secret_key=$S_MEMCACHE"
    --set "endpoints.oslo_messaging.auth.admin.password=$S_RABBIT_ADMIN"
    --set "endpoints.oslo_messaging.auth.cloudkitty.password=$S_CLOUDKITTY_RABBIT"
)
...
if "${helm_command[@]}" "${HELM_PASS_THROUGH[@]}"; then
    echo "Helm upgrade successful. Waiting for Cloudkitty deployments..."
    kubectl -n "$SERVICE_NAMESPACE" wait --for=condition=available --timeout=300s \
        deployment/cloudkitty-api \
        deployment/cloudkitty-processor
