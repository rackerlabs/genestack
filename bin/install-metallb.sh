...
if execute_helm_upgrade helm_command HELM_PASS_THROUGH; then
    echo "Helm upgrade successful. Waiting for MetalLB components..."
    # Check the Controller (Deployment) and Speakers (DaemonSet)
    wait_for_resource_ready "$SERVICE_NAMESPACE" deployment 300 controller
    wait_for_resource_ready "$SERVICE_NAMESPACE" daemonset 300 speaker
