...
if execute_helm_upgrade helm_command HELM_PASS_THROUGH; then
    echo "Helm upgrade successful. Waiting for Loki to be ready..."
    # Loki can be a StatefulSet or Deployment depending on the chart mode. 
    # We check for the most common component.
    if kubectl -n "$SERVICE_NAMESPACE" get statefulset loki >/dev/null 2>&1; then
        wait_for_resource_ready "$SERVICE_NAMESPACE" statefulset 300 loki
    elif kubectl -n "$SERVICE_NAMESPACE" get deployment loki-read >/dev/null 2>&1; then
        wait_for_resource_ready "$SERVICE_NAMESPACE" deployment 300 loki-read
    fi
