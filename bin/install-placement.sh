...
SERVICE_NAME_DEFAULT="placement"
...
S_PLACEMENT_ADMIN=$(get_or_create_secret "$SERVICE_NAMESPACE" "placement-admin" "password" 32 "$ROTATE_SECRETS")
S_MARIADB_ROOT=$(get_or_create_secret "$SERVICE_NAMESPACE" "mariadb" "root-password" 32 "$ROTATE_SECRETS")
S_PLACEMENT_DB=$(get_or_create_secret "$SERVICE_NAMESPACE" "placement-db-password" "password" 32 "$ROTATE_SECRETS")
...
set_args=(
    ...
    --set "endpoints.identity.auth.placement.password=$S_PLACEMENT_ADMIN"
    --set "endpoints.oslo_db.auth.admin.password=$S_MARIADB_ROOT"
    --set "endpoints.oslo_db.auth.placement.password=$S_PLACEMENT_DB"
    --set "endpoints.oslo_db.auth.nova_api.password=$S_NOVA_DB"
    --set "endpoints.oslo_cache.auth.memcache_secret_key=$S_MEMCACHE"
    --set "conf.placement.keystone_authtoken.memcache_secret_key=$S_MEMCACHE"
)
...
if execute_helm_upgrade helm_command HELM_PASS_THROUGH; then
    echo "Helm upgrade successful. Waiting for Placement API..."
    wait_for_resource_ready "$SERVICE_NAMESPACE" deployment 300 placement-api
