...
# Read ironic version from helm-chart-versions.yaml
...
# Extract ironic version using grep and sed
IRONIC_VERSION=$(grep 'ironic:' "$VERSION_FILE" | sed 's/.*ironic: *//')

if [ -z "$IRONIC_VERSION" ]; then
    echo "Error: Could not extract ironic version from $VERSION_FILE"
    exit 1
fi

HELM_CMD="helm upgrade --install ironic openstack-helm/ironic --version ${IRONIC_VERSION} \
    --namespace=openstack \
    --timeout 120m"
