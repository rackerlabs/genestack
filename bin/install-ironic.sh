...
S_IRONIC_ADMIN=$(get_or_create_secret "$SERVICE_NAMESPACE" "ironic-admin" "password" 32 "$ROTATE_SECRETS")
S_GLANCE_ADMIN=$(get_or_create_secret "$SERVICE_NAMESPACE" "glance-admin" "password" 32 "$ROTATE_SECRETS")
S_MARIADB_ROOT=$(get_or_create_secret "$SERVICE_NAMESPACE" "mariadb" "root-password" 32 "$ROTATE_SECRETS")
S_IRONIC_DB=$(get_or_create_secret "$SERVICE_NAMESPACE" "ironic-db-password" "password" 32 "$ROTATE_SECRETS")
...
S_IRONIC_RABBIT=$(get_or_create_secret "$SERVICE_NAMESPACE" "ironic-rabbitmq-password" "password" 64 "$ROTATE_SECRETS")

set_args=(
    --set "endpoints.identity.auth.admin.password=$S_KEYSTONE_ADMIN"
    --set "endpoints.identity.auth.ironic.password=$S_IRONIC_ADMIN"
    --set "endpoints.identity.auth.glance.password=$S_GLANCE_ADMIN"
    --set "endpoints.oslo_db.auth.admin.password=$S_MARIADB_ROOT"
    --set "endpoints.oslo_db.auth.ironic.password=$S_IRONIC_DB"
    --set "endpoints.oslo_cache.auth.memcache_secret_key=$S_MEMCACHE"
    --set "conf.ironic.keystone_authtoken.memcache_secret_key=$S_MEMCACHE"
    --set "conf.ironic.database.slave_connection=mysql+pymysql://ironic:$S_IRONIC_DB@mariadb-cluster-secondary.openstack.svc.cluster.local:3306/ironic"
    --set "endpoints.oslo_messaging.auth.admin.password=$S_RABBIT_ADMIN"
    --set "endpoints.oslo_messaging.auth.ironic.password=$S_IRONIC_RABBIT"
)
...
if "${helm_command[@]}" "${HELM_PASS_THROUGH[@]}"; then
    echo "Helm upgrade successful. Waiting for Ironic deployments..."
    kubectl -n "$SERVICE_NAMESPACE" wait --for=condition=available --timeout=300s \
        deployment/ironic-api \
        deployment/ironic-conductor
    
    # Optional Inspector check if it exists in the release
    if kubectl -n "$SERVICE_NAMESPACE" get deployment ironic-inspector >/dev/null 2>&1; then
        kubectl -n "$SERVICE_NAMESPACE" wait --for=condition=available --timeout=300s deployment/ironic-inspector
    fi
