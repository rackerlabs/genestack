...
process_gateway_routes() {
    ...
    for route in $routes_list; do
        ...
        if [[ -n "$route_file" ]]; then
            ...
            for gw_type in $gateway_types; do
                local gw_instance_name="${gateway_name}-${gw_type}"
                local output_file="/etc/genestack/gateway-api/routes/${gateway_name}/${route}-${gw_type}-gateway-route.yaml"
                ...
                # Remove 'custom-' prefix from HTTPRoute metadata name and add gateway type suffix
                sed -i "0,/^  name: /{s/^  name: custom-\(.*\)$/  name: \1-${gw_type}/;t;s/^  name: \(.*\)$/  name: \1-${gw_type}/}" "/tmp/${route}-${gateway_name}-${gw_type}.yaml"
