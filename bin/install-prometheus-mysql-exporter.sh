...
# Note: The exporter needs root access to fetch all performance schemas and metrics
S_DB_ROOT=$(get_or_create_secret "$SERVICE_NAMESPACE" "mariadb" "root-password" 32 "$ROTATE_SECRETS")

set_args=(
    --set "mysql.pass=$S_DB_ROOT"
)
...
if "${helm_command[@]}" "${HELM_PASS_THROUGH[@]}"; then
    echo "Helm upgrade successful. Waiting for MySQL Exporter..."
