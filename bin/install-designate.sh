...
SERVICE_NAME_DEFAULT="designate"
...
set_args=(
    ...
    --set "endpoints.oslo_db.auth.designate.password=$(kubectl --namespace openstack get secret designate-db-password -o jsonpath='{.data.password}' | base64 -d)"
    --set "endpoints.oslo_messaging.auth.admin.password=$(kubectl --namespace openstack get secret rabbitmq-default-user -o jsonpath='{.data.password}' | base64 -d)"
    --set "endpoints.oslo_messaging.auth.designate.password=$(kubectl --namespace openstack get secret designate-rabbitmq-password -o jsonpath='{.data.password}' | base64 -d)"
    ...
    --set "conf.designate.keystone_authtoken.memcache_secret_key=$(kubectl --namespace openstack get secret os-memcached -o jsonpath='{.data.memcache_secret_key}' | base64 -d)"
)
