...
S_MASAKARI_ADMIN=$(get_or_create_secret "$SERVICE_NAMESPACE" "masakari-admin" "password" 32 "$ROTATE_SECRETS")
S_MARIADB_ROOT=$(get_or_create_secret "$SERVICE_NAMESPACE" "mariadb" "root-password" 32 "$ROTATE_SECRETS")
S_MASAKARI_DB=$(get_or_create_secret "$SERVICE_NAMESPACE" "masakari-db-password" "password" 32 "$ROTATE_SECRETS")
...
S_MASAKARI_RABBIT=$(get_or_create_secret "$SERVICE_NAMESPACE" "masakari-rabbitmq-password" "password" 64 "$ROTATE_SECRETS")

set_args=(
    --set "endpoints.identity.auth.admin.password=$S_KEYSTONE_ADMIN"
    --set "endpoints.identity.auth.masakari.password=$S_MASAKARI_ADMIN"
    --set "endpoints.oslo_db.auth.admin.password=$S_MARIADB_ROOT"
    --set "endpoints.oslo_db.auth.masakari.password=$S_MASAKARI_DB"
    --set "endpoints.oslo_cache.auth.memcache_secret_key=$S_MEMCACHE"
    --set "conf.masakari.keystone_authtoken.memcache_secret_key=$S_MEMCACHE"
    --set "endpoints.oslo_messaging.auth.admin.password=$S_RABBIT_ADMIN"
    --set "endpoints.oslo_messaging.auth.masakari.password=$S_MASAKARI_RABBIT"
)
...
if execute_helm_upgrade helm_command HELM_PASS_THROUGH; then
    echo "Helm upgrade successful. Waiting for Masakari API..."
    wait_for_resource_ready "$SERVICE_NAMESPACE" deployment 300 masakari-api
