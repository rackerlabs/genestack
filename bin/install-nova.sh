...
# Read nova version from helm-chart-versions.yaml
...
# Extract nova version using grep and sed
NOVA_VERSION=$(grep 'nova:' "$VERSION_FILE" | sed 's/.*nova: *//')

if [ -z "$NOVA_VERSION" ]; then
    echo "Error: Could not extract nova version from $VERSION_FILE"
    exit 1
fi

HELM_CMD="helm upgrade --install nova openstack-helm/nova --version ${NOVA_VERSION} \
    --namespace=openstack \
    --timeout 120m"
