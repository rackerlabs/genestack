...
S_KEYSTONE_DB=$(get_or_create_secret "$SERVICE_NAMESPACE" "keystone-db-password" "password" 32 "$ROTATE_SECRETS")
...
set_args=(
    ...
    --set "endpoints.oslo_db.auth.keystone.password=$S_KEYSTONE_DB"
    --set "endpoints.oslo_messaging.auth.admin.password=$S_RABBIT_ADMIN"
    --set "endpoints.oslo_messaging.auth.keystone.password=$S_KEYSTONE_RABBIT"
)
...
if "${helm_command[@]}" "${HELM_PASS_THROUGH[@]}"; then
    echo "Helm upgrade successful. Waiting for Keystone deployment..."
    kubectl -n "$SERVICE_NAMESPACE" wait --for=condition=available --timeout=300s deployment/keystone-api
