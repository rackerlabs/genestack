...
# Read cinder version from helm-chart-versions.yaml
...
# Extract cinder version using grep and sed
CINDER_VERSION=$(grep 'cinder:' "$VERSION_FILE" | sed 's/.*cinder: *//')

if [ -z "$CINDER_VERSION" ]; then
    echo "Error: Could not extract cinder version from $VERSION_FILE"
    exit 1
fi

HELM_CMD="helm upgrade --install cinder openstack-helm/cinder --version ${CINDER_VERSION} \
    --namespace=openstack \
    --timeout 120m"
