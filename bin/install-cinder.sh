...
S_CINDER_DB=$(get_or_create_secret "$SERVICE_NAMESPACE" "cinder-db-password" "password" 32 "$ROTATE_SECRETS")
...
set_args=(
    ...
    --set "endpoints.oslo_db.auth.cinder.password=$S_CINDER_DB"
    --set "endpoints.oslo_cache.auth.memcache_secret_key=$S_MEMCACHE"
    --set "conf.cinder.keystone_authtoken.memcache_secret_key=$S_MEMCACHE"
    --set "conf.cinder.database.slave_connection=mysql+pymysql://cinder:$S_CINDER_DB@mariadb-cluster-secondary.openstack.svc.cluster.local:3306/cinder"
    --set "endpoints.oslo_messaging.auth.admin.password=$S_RABBIT_ADMIN"
    --set "endpoints.oslo_messaging.auth.cinder.password=$S_CINDER_RABBIT"
)
...
if execute_helm_upgrade helm_command HELM_PASS_THROUGH; then
    echo "Helm upgrade successful. Waiting for Cinder deployments..."
    wait_for_resource_ready "$SERVICE_NAMESPACE" deployment 300 cinder-api cinder-scheduler cinder-volume
