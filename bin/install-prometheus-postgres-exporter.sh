...
# Retrieving the administrative password for the postgres user
S_PG_PASS=$(get_or_create_secret "$SERVICE_NAMESPACE" "postgres-password" "password" 32 "$ROTATE_SECRETS")

set_args=(
    --set "config.datasource.password=$S_PG_PASS"
)
...
if "${helm_command[@]}" "${HELM_PASS_THROUGH[@]}"; then
    echo "Helm upgrade successful. Waiting for Postgres Exporter..."
