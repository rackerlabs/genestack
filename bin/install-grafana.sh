...
# We manage the admin password explicitly to prevent Helm re-generating it on every upgrade
...
set_args=(
    --set "adminPassword=$S_GRAFANA_ADMIN"
)
...
if "${helm_command[@]}" "${HELM_PASS_THROUGH[@]}"; then
    echo "Helm upgrade successful. Waiting for Grafana deployment..."
    kubectl -n "$SERVICE_NAMESPACE" wait --for=condition=available --timeout=300s deployment/grafana
