...
!!! info "What is a Management Cluster?"
    The management cluster is a Kubernetes cluster that hosts Cluster API controllers. It manages the lifecycle of workload clusters, handling provisioning, scaling, and upgrades.

### :material-check-decagram: Prerequisites Check

Verify Python 3 is installed on all master nodes.
...
```bash
root@k8s-master01:~# apt-get install python3.10-venv
```
...
Create a tmux session to ensure the installation continues even if the connection drops.

```bash
root@k8s-master01:~# tmux new -s capi-mgmt-cluster
```
...
Clone the Kubespray repository to `/opt/kubespray`.
...
Create and activate a Python virtual environment for Kubespray.

```bash
root@k8s-master01:~# python3 -m venv ~/.venvs/kubespray-venv
root@k8s-master01:~# source ~/.venvs/kubespray-venv/bin/activate
```
...
Navigate to the Kubespray directory and install required Python packages.
...
Create the `inventory.ini` file with your cluster node configuration.
...
Create the necessary configuration files for the cluster.
...
Execute the Kubespray playbook to deploy the Kubernetes cluster.
...
Check that all nodes are ready and running.
...
Prepare the kubeconfig file (`capi-mgmt-cluster.kubeconfig`) for remote access from the controller node.
...
Create a test pod to verify DNS functionality.
...
Download and install the clusterctl CLI tool.
...
Deploy the OpenStack Resource Controller (ORC) to manage OpenStack resources from Kubernetes.
...
Initialize Cluster API with the OpenStack infrastructure provider.
...
Check that all CAPI components are running:
...
Add the CAPI addons Helm repository and install the addon provider.
...
Check the load balancer statistics to ensure all backend servers are healthy.
...
Verify the management cluster nodes are ready and operational.
...
Verify all CAPI controllers are running in the management cluster.
...
=== "Critical Components"
    ...
    | **OpenStack Provider** (1) | `capo-system` | Provisions infrastructure on OpenStack |
...
Verify all OpenStack Helm releases are deployed and operational.
...
Check MariaDB cluster health and synchronization status.
...
Verify RabbitMQ cluster status and node connectivity.
...
Check Magnum API and conductor pods are running.
...
Verify Magnum API is properly exposed.
...
## Installing Magnum with CAPI Support
...
Modify the Magnum Helm chart version to enable CAPI support.
...
Confirm the upgrade was successful and new pods are running.
...
```mermaid
graph TB
    A[Management Cluster<br/>Kubespray + CAPI] --> B[CAPI Controllers]
    B --> C[OpenStack Provider<br/>CAPO]
    C --> D[OpenStack Cloud]
    D --> E[Magnum Service]
    E --> F[Workload Clusters]
    
    G[Load Balancer<br/>HAProxy] --> A
    H[MariaDB Galera<br/>3 nodes] --> D
    I[RabbitMQ Cluster<br/>3 nodes] --> D
...
Upgrade Kubernetes version (if supported by template).
