...
``` shell
ubuntu@freezer-client:~$ sudo apt-get install python3-dev
ubuntu@freezer-client:~$ sudo apt install python3.12-venv
ubuntu@freezer-client:~$ sudo python3 -m venv freezer-venv
ubuntu@freezer-client:~$ source freezer-venv/bin/activate

(freezer-venv) ubuntu@freezer-client:~$ pip install pymysql
(freezer-venv) ubuntu@freezer-client:~$ pip install freezer
```

Now freezer binaries are available:

``` shell
(freezer-venv) ubuntu@freezer-client:~$ freezer #press TAB
freezer            freezer-agent      freezer-scheduler
```
...
Create `freezer-schduler.conf` file
``` shell
---
[DEFAULT]
...
``` shell
(freezer-venv) ubuntu@freezer-client:~$ freezer-scheduler start \
                                        --insecure \
...
(freezer-venv) ubuntu@freezer-client:~$ freezer client-register --file client_register_config.json
...
``` shell
ubuntu@hyperconverged-0:~$ sudo kubectl logs -n openstack freezer-api-6849445b5c-7s4cn -f
ubuntu@hyperconverged-0:~$ sudo kubectl logs -n openstack freezer-api-6849445b5c-dqm8n -f
...
...
(freezer-venv) ubuntu@freezer-client:~$ freezer job-create \
                                        --file temp-job.json \
                                        -C backup-client-vm \
...
(freezer-venv) ubuntu@freezer-client:~$ freezer job-list
+--------+-------------+-----------+--------+-----------+-------+------------+
...
(freezer-venv) ubuntu@freezer-client:~$ freezer job-show 9999
+-------------+--------------------------------------------------------------+
| Field       | Value                                                        |
+-------------+--------------------------------------------------------------+
| Job ID      | 9999                                                         |
...
(freezer-venv) ubuntu@freezer-client:~$ freezer job-update 9999 temp-job.json 
Job 9999 updated

(freezer-venv) ubuntu@freezer-client:~$ freezer job-start 9999
...
``` shell
(freezer-venv) ubuntu@freezer-client:~$ freezer-agent \
                                        --action backup \
...
``` shell
(freezer-venv) ubuntu@freezer-client:~$ tree /home/ubuntu/freezer-bkp-qcow/
...
(freezer-venv) ubuntu@freezer-client:~$ sudo systemctl stop mongod
(freezer-venv) ubuntu@freezer-client:~$ sudo systemctl status mongod
...
(freezer-venv) ubuntu@freezer-client:~$ sudo systemctl start mongod
(freezer-venv) ubuntu@freezer-client:~$ sudo systemctl status mongod
```
...
(freezer-venv) ubuntu@freezer-client:~$ mkdir -p /tmp/mongodb-restore
...
(freezer-venv) ubuntu@freezer-client:~$ swift list --lh freezer-bkp-mongo-lvm
...
(freezer-venv) ubuntu@freezer-client:~$ sudo rsync -a /tmp/mongodb-restore/ /var/lib/mongodb/
(freezer-venv) ubuntu@freezer-client:~$ sudo chown -R mongodb:mongodb /var/lib/mongodb
...
``` shell
(freezer-venv) ubuntu@freezer-client:~$ sudo systemctl stop mysql
(freezer-venv) ubuntu@freezer-client:~$ sudo systemctl status mysql

(freezer-venv) ubuntu@freezer-client:~$ mkdir -p /tmp/mysql-backup
...
(freezer-venv) ubuntu@freezer-client:~$ sudo rsync -a /tmp/mysql-backup/ /var/lib/mysql/
(freezer-venv) ubuntu@freezer-client:~$ sudo chown -R mysql:mysql /var/lib/mysql

(freezer-venv) ubuntu@freezer-client:~$ sudo systemctl start mysql.service
(freezer-venv) ubuntu@freezer-client:~$ sudo systemctl status mysql.service
```
...
(freezer-venv) ubuntu@freezer-client:~$ mkdir -p /tmp/mysql-restore

# Restore timestamp can be fetched by querying the swift objects in the backup container like below.
# Swift authenticates using the S3 credentials available on Freezer-client VM.

(freezer-venv) ubuntu@freezer-client:~$ swift list --lh freezer-bkp-mysql
...
(freezer-venv) ubuntu@freezer-client:~$ sudo rsync -a /tmp/mysql-restore/ /var/lib/mysql/
(freezer-venv) ubuntu@freezer-client:~$ sudo chown -R mysql:mysql /var/lib/mysql
...
(freezer-venv) ubuntu@freezer-client:~$ sudo systemctl start mysql
(freezer-venv) ubuntu@freezer-client:~$ sudo systemctl status mysql
...
``` shell
(freezer-venv) ubuntu@freezer-client:~$ cat session_file.json 
{
...
(freezer-venv) ubuntu@freezer-client:~$ freezer session-create --file session_file.json 
Session c6a7fcfd06134b788f0a34e8454174b1 created

(freezer-venv) ubuntu@freezer-client:~$ freezer session-list
+----------------------------------+---------------------------------+--------+--------+--------+
...
(freezer-venv) ubuntu@freezer-client:~$ freezer session-add-job \
        --session-id c6a7fcfd06134b788f0a34e8454174b1 \
        --job-id 9999

# Start a session with a specific job (job-* are arbitrary):

(freezer-venv) ubuntu@freezer-client:~$ freezer session-start \
        --session-id c6a7fcfd06134b788f0a34e8454174b1 \
        --job-id 9999 \
        --job-tag 0
