...
Obtain the current Unix time by running `date +%s`.

Set an object to expire at an absolute (UNIX) time:
``` shell
swift post flex-container01 document01.rtf -H "X-Delete-At:UNIX_TIME"
```
