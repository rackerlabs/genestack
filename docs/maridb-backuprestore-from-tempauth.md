...
This document provides procedures to restore MariaDB backups stored in Rackspace's Swift object storage with tempauth for the production environments: DFW, SJC, and IAD. It details two methods: using theKubernetes Restore CRD with the MariaDB Operator and a manual restore using AWS S3 commands. These procedures ensure recovery from backups in the mariadb-backups container, based on the reference document present.
...
## 2. Restore Flow
![MariaDbRestore FlowDiagram](assets/images/MariaDBRestoreFlowDiagram.png)
...
## 6. Escalation:
  If validation fails, coordinate with James Denton or Jorge Perez to resolve network or any configuration issues.
