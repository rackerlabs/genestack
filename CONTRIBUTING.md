...
5. All commits altering previous behavior need to be accompanied by release notes within the [releasenotes](/releasenotes) folder, the format is are generated by reno.
   
   a) Any variable or change altering services must be pointed out inside inside the upgrade section of the release notes.
...
## Merging into stable branches

1. Changes that do not break existing services or alter behavior can be automatially cherry-picked from newer branches
   once those have been peer reviewed.
2. Bugfixes, if applicable, should be cherry picked and encouraged into stable branches.
3. New features can be merged into stable branches when accompanied by release notes

   a) Those features must independent and free of dependencies to keep the impact as low as possible
